...
export default function NewsList({ data, loading, isOffline, onLoadMore }) {
  ...
  return (
    <div style={{ minHeight: "100vh", color: theme.text }}>
      ...
      >
        <h1 className="hero-title">📰 Smart News Explorer</h1>
        ...
        >
          ...
          {/* API TIME BADGE (Always visible if data exists) */}
          {data?.timeTakenMs && (
            <div
              style={{
                fontSize: 13,
      ...
      >
        {loading
          ? Array.from({ length: 6 }).map((_, i) => (
              <div
                key={i}
                className={`news-skeleton ${darkMode ? "dark" : ""}`}
              >
      ...
      {/* Loading footer */}
      {loading && (
        <div style={{ textAlign: "center", padding: 20 }}>Loading more news...</div>
      )}
    </div>
  );
}